...
void demo(char *cfgfile, char *weightfile, float thresh, float hier_thresh, int cam_index, const char *filename, char **names, int classes, int avgframes,
    ...
    int benchmark, int benchmark_layers, char *img_sequence_dir, char *output_dir)
{
    ...
    while(1){
        ++count;
        {
            ...
            if (!benchmark && !dontdraw_bbox) draw_detections_cv_v3(show_img, local_dets, local_nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes, demo_ext_output, img_sequence_dir, output_dir);
